...
public class feeffe
{
	...
	async static void DownloadTest() {
		...
		try {
			...
			var success = await Utility.DownloadAsync(fileUrl, fileLocation, System.Threading.CancellationToken.None);
			Log.Info("finished");
		...
		foreach (var file in dir) {
			...
			var writeStream = FileSystem.Root.OpenWrite($"data/{file.FilenameInZip}", System.IO.FileMode.CreateNew);
